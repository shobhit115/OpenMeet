...
export default function RemoteVideo({ stream, username, isPinned, onPin, isPresentation }) {
    ...
    // Format the display name properly based on whether it's a camera feed or a presentation
    const baseUsername = username || "User";
    const displayName = isPresentation ? `${baseUsername}'s presentation` : baseUsername;
        ...
        <div className={`relative bg-gray-900 rounded-2xl overflow-hidden shadow-xl flex items-center justify-center w-full h-full group transition-all duration-300 ${
            ...
            {stream ? (
                <video
                    ...
                    playsInline
                    /* 
                      - object-contain: prevents screen shares from being cropped/zoomed in.
                      - transform scale-x-[-1]: mirrors camera feed horizontally, but is omitted for screen shares.
                    */
            ...
            <div className="absolute bottom-3 left-3 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/10 text-xs font-medium text-white flex items-center gap-2 z-[5] shadow-md">
                <span className="truncate max-w-[160px]">
                    {username ? displayName : "Connecting..."}
                </span>
            </div>
